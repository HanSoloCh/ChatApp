...
void MainWindow::slotShowFile(const QString& nickname, const QString& fileName,
    ...
    QPushButton* downloadButton = new QPushButton(
        QString("%1 send %2").arg(nickname, QFileInfo(fileName).baseName()), ui->listWidget);
    ui->listWidget->setItemWidget(item, downloadButton);
    connect(downloadButton, &QPushButton::clicked, [fileName] {
        QString savePath =
            QFileDialog::getSaveFileName(nullptr, "Save File", QFileInfo(fileName).fileName());
        if (!savePath.isEmpty()) {
...
void MainWindow::slotSendMessage() {
    ...
    TextMessageData messageData(UserMessage, getNickname(), ui->messageEdit->text(), messageId,
                                ui->spinBox->value());
...
void MainWindow::slotSendFile() {
    ...
    if (!fileName.isEmpty()) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            FileMessageData messageData(UserFile, getNickname(), file, messageId,
                                        ui->spinBox->value());
            emit sendMessage(messageData, qMakePair(ui->senderIP->text(), ui->senderPort->value()));
            file.close();
        }
