...
Client::Client(quint16 curPort, QObject* parent) : QObject(parent), port(curPort) {
    ...
    connect(receivedMessageManager, &ReceivedMessageManager::textMessageComplete, this,
            &Client::slotTextMessageComplete);
    connect(receivedMessageManager, &ReceivedMessageManager::fileMessageComplete, this,
            &Client::slotFileMessageComplete);

    connect(receivedMessageManager, &ReceivedMessageManager::notifyClientMessageReceived, this,
            &Client::slotNotifyClientMessageReceived);
    connect(receivedMessageManager, &ReceivedMessageManager::requestMissingPart, this,
            &Client::slotRequestMissingPart);
}
...
void Client::processIncomingMessage(const Message& message, const UserAddres& sender) {
    if (message.getMessageType() == SystemMessageReceived) {
        ...
        sendQueue.enqueue(
            sendMessageManager->getMessage(message.getMessageId(), message.getIndex()));
    } else {
...
void Client::slotMessageReceived(const QUuid& messageId) { emit showMessageReceived(messageId); }
...
void Client::slotSendMessage(const BaseMessageData& messageData, const UserAddres& addres) {
    ...
    for (int partIndex = 0; partIndex < totalParts; ++partIndex) {
        Message::MessageHeader header(messageData.type(), messageData.getId(), partIndex,
                                      totalParts);
