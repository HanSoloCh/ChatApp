...
QDataStream& operator<<(QDataStream& out, const Message::MessageHeader& messageHeader) {
    out << static_cast<quint8>(messageHeader.type) << messageHeader.messageId
        << messageHeader.partIndex << messageHeader.totalPartsCount;
    return out;
}

QDataStream& operator>>(QDataStream& in, Message::MessageHeader& messageHeader) {
    quint8 type;
    in >> type >> messageHeader.messageId >> messageHeader.partIndex >>
...
Message::Message(MessageType curType, QUuid id, quint32 index, quint32 count,
...
Message::Message(const Message::MessageHeader& head, const QByteArray& data)
    : header(head), messageData(data){};
